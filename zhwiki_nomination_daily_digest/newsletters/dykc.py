...
from ..parsers.dykc import DYKEntry, DYKCDiffReport
...
REMOVED_ICON_PASSED = r'[[File:Yes check.svg|15px|link=|alt=通過|class=notpageimage]]'
REMOVED_ICON_FAILED = r'[[File:X mark.svg|15px|link=|alt=不通過|class=notpageimage]]'
...
def generate_votes_display(entry: DYKEntry) -> str:
...
def generate_votes_delta_display(delta: DYKEntry) -> str:
...
def generate_newsletter_content(diff_report: DYKCDiffReport) -> Optional[str]:
    ...
    # New entries
    rows.append(
        f"\n[[File:Codex icon bellOutline.svg|20px|alt=新增條目|class=skin-invert notpageimage]]&nbsp;'''新增條目：'''共有{len(diff_report.new_entries)}個新條目提交評選：")
    for title, entry in diff_report.new_entries.items():
        linkicon = DYKC_LINK_ICON.format(pagename=title)
        title_beautified = title.replace('_', ' ')
        row = f"* {linkicon}&nbsp;[[{title_beautified}]]：{generate_votes_display(entry)}"
        rows.append(row)

    # Vote Differences
    rows.append(
        f"\n[[File:Codex icon add.svg|20px|alt=票數變化|class=skin-invert notpageimage]]&nbsp;'''票數變化：'''共有{len(diff_report.vote_differences)}個新條目有票數變化：")
    for title, delta in diff_report.vote_differences.items():
        entry = diff_report.new_votes.get(title)
        linkicon = DYKC_LINK_ICON.format(pagename=title)
        title_beautified = title.replace('_', ' ')
        row = f"* {linkicon}&nbsp;[[{title_beautified}]]：{generate_votes_display(entry)} {generate_votes_delta_display(delta)}"
        rows.append(row)

    # Removed entries
    rows.append(
        f"\n[[File:Codex icon checkAll.svg|20px|alt=完成評選|class=skin-invert notpageimage]]&nbsp;'''完成評選：'''共有{len(diff_report.removed_entries)}個新條目評選已完結：")
    for title, passed in diff_report.removed_entries.items():
        title_beautified = title.replace('_', ' ')
        icon = REMOVED_ICON_PASSED if passed else REMOVED_ICON_FAILED
        result = REMOVED_RESULT_PASSED if passed else REMOVED_RESULT_FAILED
        row = REMOVED_ROW.format(
            icon=icon, pagename=title_beautified, result=result)
        rows.append(row)
